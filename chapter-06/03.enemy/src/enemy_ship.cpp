...
void EnemyShip::Decelerate() {
  m_DX -= ( m_DX * delta_time ) / 2.0;
  m_DY -= ( m_DY * delta_time ) / 2.0;
}
...
void EnemyShip::Move() {
  ...
  if ( m_AIState == TURN_LEFT ) {
    RotateLeft();
  } else if ( m_AIState == TURN_RIGHT ) {
    RotateRight();
  } else if ( m_AIState == ACCELERATE ) {
    Accelerate();
  } else if ( m_AIState == DECELERATE ) {
    Decelerate();
  }
...
void EnemyShip::Render() {
  ...
  int return_code = SDL_RenderCopyEx( renderer, m_SpriteTexture, NULL, &dest, degrees, NULL, SDL_FLIP_NONE );
  if( return_code != 0 ) {
